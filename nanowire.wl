...
wfmajoranadis[a_,\[Mu]_,vz_,vimp_,dim_,index_]:=Block[{testp,testn,gamma,test,gamma1,gamma2},If[Length@vimp!=dim,Message[wfmajoranambdis::nnarg,Length@vimp,dim];Throw[$Failed]];testp=Transpose[Sort[Select[Transpose[Eigensystem[Hdis[a,\[Mu],vz,vimp,dim],-20,Method->{"Arnoldi","Criteria"->"Magnitude","MaxIterations"->\[Infinity]}]],#[[1]]>0&],#1[[1]]<#2[[1]]&][[1;;6]]];gamma1=Table[gamma=KroneckerProduct[1/2 ConjugateTranspose@{{1,I,0,0},{0,0,1,I},{0,0,1,-I},{-1,I,0,0}},IdentityMatrix[dim,SparseArray]].testp[[2,i]];Total[ArrayReshape[Abs[gamma]^2,{4,dim}][[{1,3}]]],{i,index}];gamma2=Table[gamma=KroneckerProduct[1/2 ConjugateTranspose@{{1,I,0,0},{0,0,1,I},{0,0,1,-I},{-1,I,0,0}},IdentityMatrix[dim,SparseArray]].testp[[2,i]];Total[ArrayReshape[Abs[gamma]^2,{4,dim}][[{2,4}]]],{i,index}];ListLinePlot[Flatten[Transpose[ArrayFlatten[{gamma1,gamma2}]],1],DataRange->{0,dim*a/100},PlotLabel->"\!\(\*SubscriptBox[\(V\), \(Z\)]\)="<>ToString[vz]<>"(meV)",PlotRange->Full,Frame->True,Filling->Axis,PlotLegends->Placed[SwatchLegend[{"1st \!\(\*SubscriptBox[\(\[Gamma]\), \(1\)]\)","1st \!\(\*SubscriptBox[\(\[Gamma]\), \(2\)]\)","2nd \!\(\*SubscriptBox[\(\[Gamma]\), \(1\)]\)","2nd \!\(\*SubscriptBox[\(\[Gamma]\), \(2\)]\)"}],Top],FrameLabel->{"x(\[Mu]m)",""},PlotStyle->{Blue,Cyan,Red,Orange}]]


Hqd[a_?NumberQ,\[Mu]0_?NumberQ,\[CapitalDelta]0_?NumberQ,V0_?NumberQ,\[Alpha]0_?NumberQ,dim0_?IntegerQ,l0_?IntegerQ,\[Mu]m_?NumberQ]:=Block[{t=25/a^2,\[Mu]=\[Mu]0,\[Alpha]=\[Alpha]0/(2a),Vz=V0,\[CapitalDelta]=\[CapitalDelta]0,l=l0,\[CapitalDelta]fun,\[Mu]fun,\[Sigma]=1,\[Mu]max=\[Mu]m,s1,s2,s3,dim=dim0},\[CapitalDelta]list=\[CapitalDelta] UnitStep[Range[dim]-l];\[Mu]list=\[Mu]-\[Mu]max Exp[-Range[dim]^2/(\[Sigma] l^2)];s1=KroneckerProduct[PauliMatrix[3],KroneckerProduct[PauliMatrix[0],-t*(SparseArray[{Band[{1,2}]->1,Band[{2,1}]->1},{dim,dim}])+(2*t)IdentityMatrix[dim,SparseArray]-DiagonalMatrix[SparseArray@\[Mu]list]]+KroneckerProduct[PauliMatrix[2],I*\[Alpha]*(SparseArray[{Band[{1,2}]->-1,Band[{2,1}]->1},{dim,dim}])]];s2=KroneckerProduct[PauliMatrix[0],KroneckerProduct[PauliMatrix[3],Vz*IdentityMatrix[dim,SparseArray]]];
s3=KroneckerProduct[PauliMatrix[1],KroneckerProduct[PauliMatrix[0],DiagonalMatrix[SparseArray@\[CapitalDelta]list]]];(s1+s2)+s3]
